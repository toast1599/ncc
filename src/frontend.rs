    ...
    fn parse_equality(&mut self) -> Result<Expr> {
        ...
        loop {
            e = match self.peek() {
                Token::EqEq => {
                    self.bump();
                    Expr::Eq(Box::new(e), Box::new(self.parse_relational()?))
                }
                Token::NotEq => {
                    self.bump();
                    Expr::Ne(Box::new(e), Box::new(self.parse_relational()?))
                }
                _ => break,
            };
        }
    ...
    fn parse_relational(&mut self) -> Result<Expr> {
        ...
        loop {
            e = match self.peek() {
                Token::Lt => {
                    self.bump();
                    Expr::Lt(Box::new(e), Box::new(self.parse_additive()?))
                }
                Token::Le => {
                    self.bump();
                    Expr::Le(Box::new(e), Box::new(self.parse_additive()?))
                }
                Token::Gt => {
                    self.bump();
                    Expr::Gt(Box::new(e), Box::new(self.parse_additive()?))
                }
                Token::Ge => {
                    self.bump();
                    Expr::Ge(Box::new(e), Box::new(self.parse_additive()?))
                }
                _ => break,
            };
        }
    ...
    fn parse_additive(&mut self) -> Result<Expr> {
        ...
        loop {
            e = match self.peek() {
                Token::Plus => {
                    self.bump();
                    Expr::Add(Box::new(e), Box::new(self.parse_multiplicative()?))
                }
                Token::Minus => {
                    self.bump();
                    Expr::Sub(Box::new(e), Box::new(self.parse_multiplicative()?))
                }
                _ => break,
            };
        }
    ...
    fn parse_multiplicative(&mut self) -> Result<Expr> {
        ...
        loop {
            e = match self.peek() {
                Token::Star => {
                    self.bump();
                    Expr::Mul(Box::new(e), Box::new(self.parse_unary()?))
                }
                Token::Slash => {
                    self.bump();
                    Expr::Div(Box::new(e), Box::new(self.parse_unary()?))
                }
                Token::Percent => {
                    self.bump();
                    Expr::Rem(Box::new(e), Box::new(self.parse_unary()?))
                }
                _ => break,
            };
        }
...
mod tests {
    ...
    fn parses_all_comparisons() {
        for op in ["==", "!=", "<", "<=", ">", ">="] {
            parse(&format!("int main(void) {{ return 2 {op} 3; }}")).unwrap();
        }
    }
    ...
    #[test]
    fn comparison_precedence_is_c_like() {
        let f = parse("int main(void) { return 1 + 2 < 4 == 1; }").unwrap();
        assert!(matches!(f.return_value, Expr::Eq(_, _)));
    }
}
